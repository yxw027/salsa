...
TEST (FeatFactor, ADvsANEval)
{
    ...
    FeatFunctor f(0.0, 3.0*Matrix2d::Identity(), xb2c, zi, zj, Vector2d::Zero(), 0);
    FeatFactor an(&f);
...
TEST (FeatFactor, FDvsANJac1)
{
    ...
    zi.normalize();
    zj.normalize();
    FeatFunctor f(0.0, Matrix2d::Identity(), xb2c, zi, zj, Vector2d(0, 0), 0);
    FeatFactor an(&f);
    xform::Xformd xi  = Xformd::Random();
    xform::Xformd xj  = Xformd::Random();
...
TEST (FeatFactor, SimulatedFeatures)
{
    ...
    for (int i = 0; i < z_.pixs.size(); i++)
    {
        ...
        FeatFunctor f(0.0, Matrix2d::Identity(), x_b2c, zeta0, zeta1, Vector2d(0, 0), 0);
...
TEST (FeatFactor, OptOnePointOneView)
{
    ...
    double rhohat = 0.001;
    FeatFunctor f(0.0, Matrix2d::Identity(), x_b2c, zeta_i, zeta_j, Vector2d(0, 0), 1);
